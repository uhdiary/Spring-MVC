...
<body>
	...
	<div id="notee">
		<ul id="tabs">
			<li><a href="#" name="#tab1">보낸 쪽지함</a></li>
		</ul>
		<div id="content">
			<div id="tab1">
				<form name="chkform">
				...
				<form action="noteSendList.do" method="post">
					...
					<input type="button" class="myButton" value="삭제" onclick="send_del()" />
				</form>
				<table id="listtable">
					...
					<thead>
						<tr style="background-color:#e6e6e6;">
							...
							<th width="5%"><label>보낸 사람</label></th>
				...
				<div id="paging">
					<%-- <a class="atag" href="noteSendList.do?page=1">[처음] </a>

					<c:if test="${startPage gt 1 }">
						<a class="atag" href="noteSendList.do?page=${startPage - 1} }">[이전]
						</a>
					</c:if>
					...
					<c:forEach var="num" begin="${startPage }" end="${endPage }"
						...
						<c:if test="${num ne currentPage }">
							<a class="numBox" href="noteSendList.do?page=${num }">${num }</a>
					</c:if>
					</c:forEach>

				<%-- 	<c:if test="${endPage lt maxPage }">
						<a class="atag" href="noteSendList.do?page=${endPage + 1}">
							[다음] </a>
					</c:if>
					...
					<a class="atag" href="noteSendList.do?page=${maxPage}"> [마지막]</a> --%>
				</div>
			</div>
		</div>
