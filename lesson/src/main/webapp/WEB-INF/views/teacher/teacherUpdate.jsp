...
<body>
	...
	<div class="nameh1">
		<span>선생님 상세 정보 수정</span>
		</div>
		<div style="height: 20px;"></div>
		<form action="teacherUpdatein.do" method="post"
			...
			<table class="table1">
				<tr>
					...
					<td><input type="text" id="tinf_titletext" name="tinf_title"
						required="required" value="${list.tinf_title}"></td>
				</tr>
				<tr>
					...
					<td><select class="select1" id="subjectid">
						...
						<input type="hidden" id="subjecttext" name="subject" value="${list.subject}">
						</td>
				</tr>
				<tr>
					<th class="thclass" id="cont3">희망 과외비</th>
					<td><input type="number" id="wish_pricetext" min="10000"
						step="10000" name="wish_price" value="${list.wish_price }">&nbsp;&nbsp;<label>원</label></td>
				</tr>
				<tr>
					<th class="thclass" id="cont4">시간당 과외비</th>
					<td><input type="number" id="time_pricetext" min="10000"
						step="10000" name="time_price" value="${list.time_price}">&nbsp;&nbsp;<label>원</label></td>
				</tr>
				<tr>
					<th class="thclass" id="cont5">학력</th>
					<td><select class="select1" id="academicselect"
						name="academic">
						<c:set var="academic">${list.academic}</c:set>	
							<option value="고등학생" <c:if test="${academic eq '고등학생'}">selected="selected"</c:if>>고등학생</option>
							<option value="방통대" <c:if test="${academic eq '방통대'}">selected="selected"</c:if>>방통대</option>
							<option value="대학교" <c:if test="${academic eq '대학교'}">selected="selected"</c:if>>대학교</option>
							<option value="대학원" <c:if test="${academic eq '대학원'}">selected="selected"</c:if>>대학원</option>
					</select></td>
				</tr>
				<tr>
					...
					<td><select class="select1" id="university_stateselect"
						...
						<c:set var="university_state">${list.university_state}</c:set>	
							<option value="재학" <c:if test="${university_state eq '재학'}">selected="selected"</c:if>>재학</option>
							<option value="휴학" <c:if test="${university_state eq '휴학'}">selected="selected"</c:if>>휴학</option>
							<option value="졸업" <c:if test="${university_state eq '졸업'}">selected="selected"</c:if>>졸업</option>
					</select></td>
				</tr>
				<tr>
					<th class="thclass" id="cont7">대학명</th>
					<td><input type="text" id="universitytext" name="university" value="${list.university}">&nbsp;&nbsp;<label>대학교</label></td>
				</tr>
				<tr>
					<th class="thclass" id="cont8">전공</th>
					<td><input type="text" id="majortext" name="major" value="${list.major}">&nbsp;&nbsp;<label>전공</label></td>
				</tr>
			
				<tr>
					<th class="thclass" id="cont10">과외경력</th>
					<td><input type="number" min="1" id="careertext" name="career" value="${list.career}">&nbsp;&nbsp;<label>년</label></td>
				</tr>
				<tr>
					...
					<td><select class="select1" id="lesson_styleid1">
						...
						<input type="hidden" id="lesson_styleid" name="lesson_style" value="${list.lesson_style }">
						</td>
				</tr>
				<tr>
					...
					<td>
						...
						<input type="hidden" id="localtext" name="lesson_area" value="${list.lesson_area }">
					</td>
				</tr>
				...
				<tr>
					...
					<td><select class="select1" id="call_timeselect"
						name="call_time">
						<c:set var="call_time">${list.call_time}</c:set>	
							<option value="시간관계없음" <c:if test="${call_time eq '시간관계없음'}">selected="selected"</c:if>>시간관계없음</option>
							<option value="오전" <c:if test="${call_time eq '오전'}">selected="selected"</c:if>>오전</option>
							<option value="오후" <c:if test="${call_time eq '오후'}">selected="selected"</c:if>>오후</option>
							<option value="저녁" <c:if test="${call_time eq '저녁'}">selected="selected"</c:if>>저녁</option>
					</select></td>
				</tr>
				<tr>
					...
					<td><select class="select1" id="taget">
						...
						<input type="hidden" id="targettext" name="lesson_target" value="${list.lesson_target}">
						</td>
				</tr>
				...
				<tr>
					...
					<td><select class="select1" id="placeselect">
						...
						<input type="hidden" id="placetext" name="place" value="${list.place }">
						</td>
				</tr>
				<tr>
					...
					<td>
						...
						<c:set var="place_check">${list.place_check}</c:set>
							<input class="check1" type="radio" name="place_check" value="가능" <c:if test="${place_check eq '가능'}">checked="checked"</c:if>>
							&nbsp;<label>가능</label>&nbsp;&nbsp;<input class="check1" type="radio"
								name="place_check" value="불가능" <c:if test="${place_check eq '불가능'}">checked="checked"</c:if>>&nbsp;<label>불가능</label>
						</div>
					</td>
				</tr>
				<tr>
					...
					<td>
						...
						<div class="checkboxfont">
						<c:set var="price_check">${list.price_check}</c:set>
							<input class="check2" type="radio" name="price_check" value="가능" <c:if test="${price_check eq '가능'}">checked="checked"</c:if>>
							&nbsp;<label>가능</label>&nbsp;&nbsp;<input class="check2" type="radio"
								name="price_check" value="불가능" <c:if test="${price_check eq '불가능'}">checked="checked"</c:if>>&nbsp;<label>불가능</label>
						</div>
				...
				<tr>
					...
					<td>
						<div class="checkboxfont">
						<c:set var="free_check">${list.free_check}</c:set>
							<input class="check3" type="radio" name="free_check" value="가능" <c:if test="${free_check eq '가능'}">checked="checked"</c:if>>
							&nbsp;<label>가능</label>&nbsp;&nbsp;<input class="check3" type="radio"
								name="free_check" value="불가능" <c:if test="${free_check eq '불가능'}">checked="checked"</c:if>>&nbsp;<label>불가능</label>
						</div>
					</td>
				</tr>
